...
def run_scan(args: argparse.Namespace, os_name: str, log_file: Path | None = None) -> list[dict]:
    ...
    if args.mode in ("files", "full", "quick"):
        print_info("[ File Analyzer ] Scanning for suspicious files, permissions, content patterns...")
        findings += file_analyzer.run(args.target, os_name, args.depth, args.verbose, log_file)
        print_success(f"File Analyzer → {len(findings)} finding(s) so far")

    if args.mode in ("files", "full", "quick"):
        print_info("[Step 4 — coming next] Hash Checker  → SHA256 + VirusTotal API")

    if args.mode in ("processes", "full"):
        print_info("[Step 5 — coming next] Process Watcher → privilege escalation, anomalies")

    if args.mode in ("network", "full"):
        print_info("[Step 6 — coming next] Network Monitor → open ports, C2 detection")

    if args.mode in ("logs", "full"):
        print_info("[Step 7 — coming next] Log Auditor → brute force, auth failures")
